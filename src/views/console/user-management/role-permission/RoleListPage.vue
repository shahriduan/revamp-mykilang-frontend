<template>
    ...
    <div class="row">
        <div class="col-12">
            <h3>Roles List</h3>
            <small class="text-muted">A role provides access to predefined menus and features so that depending on the assigned role a user can have access to what he need.</small>
        </div>
    </div>
    ...
    <div class="row mt-1 gy-4" v-if="loader === false">
        <div class="col-sm-6 col-lg-6 col-xl-4" v-show="$can('role.add')">
            <router-link :to="{ name: 'console:role.form', params: { action: 'create' }}" style="text-decoration: none;">
                ...
                <div class="card-body role-background">
                    ...
                    <div class="d-flex justify-content-between align-items-end mt-1">
                        <div class="role-heading">
                            ...
                            <router-link :to="{ name: 'console:role.form', params: { action: 'edit', 'role': role.id } }" v-show="$can('role.edit')">
                                <span>Edit Permissions</span>
                            </router-link>
                            <span class="mx-1" v-show="role.users_count == 0 && $can('role.delete')">|</span>
                            <a href="javascript:;" v-show="role.users_count == 0 && $can('role.delete')" @click="deleteRole('Delete ' + role.name + ' role', role.id)">
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <!-- <div v-show="$can('user.add') || $can('user.edit')">
                        <input type="search" class="form-control form-control-lg" placeholder="Assign user into this role" v-model="searchUser" @keyup="debounceSearchSuggestion">
                        <ul class="list-group list-group-light mt-2" v-if="searchedUsers.length > 0">
                            <li class="list-group-item d-flex justify-content-between align-items-center list-group-item-light mb-1 px-3 py-2 border-0 rounded-3" v-for="searchedUser in searchedUsers">
                                {{ searchedUser.fullname }}
                                <button type="button" class="btn btn-sm btn-outline-dark btn-rounded" @click="assignRole(modalRoleId, searchedUser.id)">
                                    Assign
                                </button>
                            </li>
                        </ul>
                        <div v-else-if="searchUser == ''">
                        </div>
                        <div v-else class="pt-1 ps-1">
                            No result found.
                        </div>
...
const debounceSearchSuggestion = useDebounceFn(async () => {
    listUserNotInRole()
}, 200)
...
async function assignRole(role_id, user_id) {
    await window.axios.post(apiRoute({ name: 'api:user_management.assign_user_role' }), {
            role_id: role_id,
            user_id: user_id
        })
        .then(async () => {
...
async function listUserNotInRole() {
    if (searchUser.value != '') {
        await window.axios.get(apiRoute({ name: 'api:user_management.list_user_not_in_role', params: { role: modalRoleId.value }, query: { search: searchUser.value } }))
            .then(function(response) {
...
async function getListUserByRoles(role_id) {
    await window.axios.get(apiRoute({ name: 'api:user_management.role_details', params: { role: role_id } }))
        .then(function(response) {
...
function deleteRole(msg, role_id) {
    confirmDialog(msg, () => {
        window.axios.post(apiRoute({ name: 'api:user_management.delete_role', params: { role: role_id } }))
            .then(function(response) {
