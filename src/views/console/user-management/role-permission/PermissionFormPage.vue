<template>
    ...
    <div class="row">
        ...
        <div class="col-12 col-sm-3 text-end">
            <BackButton :route="{ name: 'console:role.index' }" />
        </div>
    </div>
                ...
                <div>
                    ...
                    <div class="list-group list-group-flush">
                        ...
                        <div class="list-group-item" v-bind:key="index" v-for="(category, index) in datas">
                            <div class="row align-items-start">
                                <div class="col-auto">
                                    <!-- <input type="checkbox" class="form-check-input" v-model="selectedCategory[category.category.replace(' ', '_')]" @change="markPermissionByCategory(category.category.replace(' ', '_'))" :disabled="category.category == 'Roles and Permissions' && !selectedCategory['User_Management']" /> -->
                                ...
                                <div class="col">
                                    <span class="text-reset d-block">
                                        {{ category.category }}
                                        <!-- <i class="fa-solid fa-circle-info ms-1" v-if="category.category == 'Roles and Permissions'" data-bs-toggle="tooltip" data-bs-placement="right" data-bs-title="Need to select User Management to be able to select Roles and Permissions"></i> -->
                                    </span>
                                    ...
                                    <div class="d-flex flex-wrap mt-2 mb-1">
                                        <div class="form-check form-check-inline me-4" v-bind:key="index" v-for="(permission, index) in category.list_permission">
                                            <!-- <input class="form-check-input" :class="'category_' + permission.category.replace(' ', '_')" type="checkbox" value="" :id="'permission' + permission.id" v-model="selectedPermission[permission.id]" :true-value="true" :false-value="false" :disabled="category.category == 'Roles and Permissions' && !selectedCategory['User_Management']" /> -->
...
async function getPermissions() {
    ...
    await window.axios.get(apiRoute({ name: 'api:user_management.permission_list' }))
...
function markPermissionByCategory(category) {
    categoryList.value.forEach(cat => {
        if (cat.category == category.replace('_', ' ')) {
            ...
            // If the "User Management" category is unchecked, uncheck the "Roles and Permissions" category
            // if (!selectedCategory.value['User_Management'] && category === 'User_Management') {
...
// function uncheckRolesAndPermissions() {
//     // Loop through the category list and uncheck the checkboxes for "Roles and Permissions" category
//     categoryList.value.forEach(category => {
//         if (category.category === 'Roles and Permissions') {
//             selectedCategory.value[category.category.replace(' ', '_')] = false

//             // Uncheck the permissions under "Roles and Permissions" category
//             category.list_permission.forEach(permission => {
//                 selectedPermission.value[permission.id] = false
//             })
//         }
//     })
// }
...
async function getRoleDetails() {
    await window.axios.get(apiRoute({ name: 'api:user_management.role_details', params: { role: currentRoute.params.role } }))
        .then(function(response) {
...
async function submit() {
    ...
    await window.axios.post(apiRoute({ name: 'api:user_management.submit_role_permission', params: parameter }), {
