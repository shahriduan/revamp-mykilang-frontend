<template>
    <div class="card-body">
        <div class="chart-container">
            <div class="availability-loss-gauge" :id="'availability-loss-gauge-' + props.parentProps.id" ref="gaugeChart"></div>
        </div>
    </div>
</template>
...
const availabilityLossPercent = computed(() => props.parentProps.data != null ? props.parentProps.data.toFixed(2) : 0)
    ...
    series: [
        {
            ...
            data: [{ 
                value: availabilityLossPercent.value,
                itemStyle: {
...
onUpdated(() => {
    if (chartInstance) {
        chartOptions.series[0].data[0].value = availabilityLossPercent.value
        chartInstance.setOption(chartOptions)
    }
})
</script>
